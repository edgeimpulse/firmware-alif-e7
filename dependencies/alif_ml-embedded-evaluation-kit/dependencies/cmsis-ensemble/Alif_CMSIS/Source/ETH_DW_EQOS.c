...
#include "ETH_DW_EQOS.h"

static DW_EQOS_DEV EQOS0 = {
    .regs = (volatile DW_EQOS_REGS *) ETH_BASE,
    .flags  = 0,
    .cb_event = NULL,
    .frame_end = NULL,
    .irq = (IRQn_Type) ETH_SBD_IRQ,
};
...
/**
  \fn          void setup_rxdesc(DW_EQOS_DEV *dev, uint32_t desc_id)
  ...
  \return      none.
*/
static void setup_rxdesc(DW_EQOS_DEV *dev, uint32_t desc_id)
{
...
/**
  \fn          void init_rx_descs(DW_EQOS_DEV *dev)
  \brief       Initialize Rx DMA descriptors.
  \param[in]   dev        Pointer to the MAC device instance
  \return      none.
*/
static void init_rx_descs(DW_EQOS_DEV *dev)
{
...
/**
  \fn          void init_tx_desc (DW_EQOS_DEV *dev)
  \brief       Initialize Tx DMA descriptors.
  \param[in]   dev        Pointer to the MAC device instance
  \return      none.
*/
static void init_tx_descs(DW_EQOS_DEV *dev)
{
    uint32_t i;
...
/**
  \fn          void init_descriptors(DW_EQOS_DEV *dev)
  \brief       Initialize DMA descriptors.
  \param[in]   dev        Pointer to the MAC device instance
  \return      none.
*/
static void init_descriptors(DW_EQOS_DEV *dev)
{
    dev->descs = dma_descs;
...
/**
  \fn          static int32_t eqos_hw_init(DW_EQOS_DEV *dev)
  \brief       Initialize the MAC hardware.
  \param[in]   dev        Pointer to the MAC device instance
  \return      \ref execution_status.
*/
static int32_t eqos_hw_init(DW_EQOS_DEV *dev)
{
    uint32_t val;
    ...
    dev->regs->MTL_RXQ0_OP_MODE &= ~(EQOS_MTL_RXQ0_OPERATION_MODE_RQS_MASK <<
                                     MTL_TXQ0_OPERATION_MODE_TQS_SHIFT);
    dev->regs->MTL_RXQ0_OP_MODE |= (rqs << MTL_TXQ0_OPERATION_MODE_TQS_SHIFT);
#endif

    /* Enable MAC RXQ */
    dev->regs->MAC_RXQ_CTRL_0 &= ~(EQOS_MAC_RXQ_CTRL0_RXQ0EN_MASK <<
                                    EQOS_MAC_RXQ_CTRL0_RXQ0EN_SHIFT);
    dev->regs->MAC_RXQ_CTRL_0 |= (EQOS_MAC_RXQ_CTRL0_RXQ0EN_ENABLED_DCB <<
                                    EQOS_MAC_RXQ_CTRL0_RXQ0EN_SHIFT);

    /* Configure RXQ control1 for routing multicast/broadcast queues
     * Note that by default, Q0 will be used.
     */
    dev->regs->MAC_RXQ_CTRL_1 |= EQOS_MAC_RXQCTRL_MCBCQEN;

    /* Configure tx and rx flow control */
    dev->regs->MAC_Q0_TX_FLOW_CTRL |= 0xffff << EQOS_MAC_Q0_TX_FLOW_CTRL_PT_SHIFT |
                                      EQOS_MAC_Q0_TX_FLOW_CTRL_TFE;
    dev->regs->MAC_RX_FLOW_CTRL |= EQOS_MAC_RX_FLOW_CTRL_RFE;

    dev->regs->MAC_CONFIG &= ~EQOS_MAC_CONFIG_JE;
    dev->regs->MAC_CONFIG |= (EQOS_MAC_CONFIG_FES | EQOS_MAC_CONFIG_DM);

    dev->regs->MAC_PACKET_FILTER |= EQOS_MAC_PACKET_FILTER_PM;

    /* Configure the DMA block */
    dev->regs->DMA_CH0_TX_CTRL |= (16 << EQOS_DMA_CH0_TX_CONTROL_TXPBL_SHIFT);

    dev->regs->DMA_CH0_RX_CTRL |= ((16 << EQOS_DMA_CH0_RX_CONTROL_RXPBL_SHIFT) |
    ...
    dev->regs->DMA_CH0_TX_CTRL |= EQOS_DMA_CH0_TX_CONTROL_ST;
    dev->regs->DMA_CH0_RX_CTRL |= EQOS_DMA_CH0_RX_CONTROL_SR;

    dev->regs->MAC_CONFIG |= (EQOS_MAC_CONFIG_RE | EQOS_MAC_CONFIG_TE);
...
/**
  \fn          int32_t Initialize (ARM_ETH_MAC_SignalEvent_t cb_event, DW_EQOS_DEV *dev)
  \brief       Initialize Ethernet MAC Device.
  \param[in]   cb_event  Pointer to \ref ARM_ETH_MAC_SignalEvent
  \param[in]   dev       Pointer to the MAC device instance
  \return      \ref execution_status
*/
static int32_t Initialize(ARM_ETH_MAC_SignalEvent_t cb_event, DW_EQOS_DEV *dev)
{
    if (!cb_event)
...
/**
  \fn          int32_t Uninitialize (DW_EQOS_DEV *dev)
  \brief       De-initialize Ethernet MAC Device.
  \param[in]   dev       Pointer to the MAC device instance
  \return      \ref execution_status
*/
static int32_t Uninitialize(DW_EQOS_DEV *dev)
{
    dev->flags &= ~ETH_INIT;
    return ARM_DRIVER_OK;
}
...
/**
  \fn          int32_t PowerControl (ARM_POWER_STATE state, DW_EQOS_DEV *dev)
  \brief       Control Ethernet MAC Device Power.
  \param[in]   state  Power state
  \param[in]   dev       Pointer to the MAC device instance
  \return      \ref execution_status
*/
static int32_t PowerControl(ARM_POWER_STATE state, DW_EQOS_DEV *dev)
{
    int32_t ret, val;
    ...
    case ARM_POWER_FULL:
        ...
        val = *((volatile uint32_t *) ETH_50M_CLK_MUX_REG);
        val |= SEL_PHY_REFCLK;
        *((volatile uint32_t *) ETH_50M_CLK_MUX_REG) = val;

        ret = eqos_hw_init(dev);
...
/**
  \fn          int32_t GetMacAddress (ARM_ETH_MAC_ADDR *ptr_addr, DW_EQOS_DEV *dev)
  \brief       Get Ethernet MAC Address.
  \param[in]   ptr_addr  Pointer to address
  \param[in]   dev       Pointer to the MAC device instance
  \return      \ref execution_status
*/
static int32_t GetMacAddress(ARM_ETH_MAC_ADDR *ptr_addr, DW_EQOS_DEV *dev)
{
    uint32_t hi, lo;
...
/**
  \fn          int32_t SetMacAddress (const ARM_ETH_MAC_ADDR *ptr_addr, DW_EQOS_DEV *dev)
  \brief       Set Ethernet MAC Address.
  \param[in]   ptr_addr  Pointer to address
  \param[in]   dev       Pointer to MAC device instance
  \return      \ref execution_status
*/
static int32_t SetMacAddress(const ARM_ETH_MAC_ADDR *ptr_addr, DW_EQOS_DEV *dev)
{
    uint32_t hi, lo;
    ...
    hi = ((ptr_addr->b[5] <<  8) |  ptr_addr->b[4]) | EQOS_MAC_ADDR_HIGH_AE;
    ...
    DW_EQOS_DEV *dev)
{
    ...
    dev->regs->MAC_PACKET_FILTER &= ~(EQOS_MAC_PACKET_FILTER_HPF | EQOS_MAC_PACKET_FILTER_HMC);
...
/**
  \fn          int32_t ReadFrame (uint8_t *frame, uint32_t len, DW_EQOS_DEV *dev)
...
*/
static int32_t ReadFrame(uint8_t *frame, uint32_t len, DW_EQOS_DEV *dev)
{
    uint32_t cur_idx;
...
/**
  \fn          uint32_t GetRxFrameSize (DW_EQOS_DEV *dev)
  \param[in]   dev    Pointer to the MAC device instance
  \brief       Get size of received Ethernet frame.
  \return      number of bytes in received frame
*/
static uint32_t GetRxFrameSize(DW_EQOS_DEV *dev)
{
    DMA_DESC *desc;
...
/**
  \fn          int32_t GetRxFrameTime (ARM_ETH_MAC_TIME *time, DW_EQOS_DEV *dev)
  \brief       Get time of received Ethernet frame.
  \param[in]   time  Pointer to time structure for data to read into
  \param[in]   dev    Pointer to the MAC device instance
  \return      \ref execution_status
*/
static int32_t GetRxFrameTime(ARM_ETH_MAC_TIME *time, DW_EQOS_DEV *dev)
{
...
/**
  \fn          int32_t GetTxFrameTime (ARM_ETH_MAC_TIME *time, DW_EQOS_DEV *dev)
  \brief       Get time of transmitted Ethernet frame.
  \param[in]   time  Pointer to time structure for data to read into
  \param[in]   dev   Pointer to the MAC device instance
  \return      \ref execution_status
*/
static int32_t GetTxFrameTime(ARM_ETH_MAC_TIME *time, DW_EQOS_DEV *dev)
{
    DMA_DESC *desc;
...
/**
  \fn          int32_t Control (uint32_t control, uint32_t arg, DW_EQOS_DEV *dev)
  ...
  \return      \ref execution_status
*/
static int32_t Control(uint32_t control, uint32_t arg, DW_EQOS_DEV *dev)
{
    uint32_t val, reg;
    ...
    case ARM_ETH_MAC_SLEEP:
        if (arg != 0) {
            ...
            dev->regs->MAC_CONFIG &= ~(EQOS_MAC_CONFIG_TE | EQOS_MAC_CONFIG_RE);
            dev->regs->MAC_INT_ENABLE |= MAC_INT_EN_PMTIE;
            dev->regs->MAC_CONFIG |= EQOS_MAC_CONFIG_RE;
            dev->regs->MAC_PMT_CTRL_STS |= (EQOS_MAC_PMT_CTRL_STS_MGKPKTEN | EQOS_MAC_PMT_CTRL_STS_PWRDWN);
        } else {
            dev->regs->MAC_INT_ENABLE &= ~MAC_INT_EN_PMTIE;
...
/**
  \fn          int32_t ControlTimer (uint32_t control, ARM_ETH_MAC_TIME *time, DW_EQOS_DEV *dev)
...
*/
static int32_t ControlTimer(uint32_t control, ARM_ETH_MAC_TIME *time, DW_EQOS_DEV *dev)
{
    return ARM_DRIVER_ERROR_UNSUPPORTED;
}

/**
  \fn          int32_t PHY_Read (uint8_t phy_addr, uint8_t reg_addr, uint16_t *data, DW_EQOS_DEV *dev)
    ...
    DW_EQOS_DEV *dev)
{
    ...
    do {
        if (!(dev->regs->MAC_MDIO_ADDR & EQOS_MAC_MDIO_ADDR_GB)) {
...
/**
  \fn          void ETH_IRQHandler (DW_EQOS_DEV *dev)
  \param[in]   dev       Pointer to the MAC device instance
  \brief       MAC instance specific part of Ethernet Interrupt handler.
*/
static void ETH_IRQHandler(DW_EQOS_DEV *dev)
{
...
{
    return Initialize(cb_event, &EQOS0);
}
...
{
    return PowerControl(state, &EQOS0);
}
...
{
    return GetMacAddress(ptr_addr, &EQOS0);
}
...
{
    return SetMacAddress(ptr_addr, &EQOS0);
}
...
{
    return SetAddressFilter(ptr_addr, num_addr, &EQOS0);
}
...
{
    return SendFrame(frame, len, flags, &EQOS0);
}
...
{
    return ReadFrame(frame, len, &EQOS0);
}
...
{
    return GetRxFrameTime(time, &EQOS0);
}
...
{
    return GetTxFrameTime(time, &EQOS0);
}
...
{
    return Control(control, arg, &EQOS0);
}
...
{
    return ControlTimer(control, time, &EQOS0);
}
...
{
    return PHY_Read(phy_addr, reg_addr, data, &EQOS0);
}
...
{
    return PHY_Write(phy_addr, reg_addr, data, &EQOS0);
}
